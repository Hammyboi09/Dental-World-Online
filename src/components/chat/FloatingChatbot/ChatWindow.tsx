...
export function ChatWindow({ onClose }: ChatWindowProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      type: 'bot',
      content: "Hi! I'm Flossy, your dental care assistant. You can ask me any dental or dentistry related questions, or use the command buttons below for quick access to information.",
      isFirstMessage: true
    }
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userMessage.startsWith('/')) {
        ...
        const cmd = chatCommands.find(c => c.command === command);
        response = cmd ? await cmd.action(args.join(' ')) : 'Unknown command. Use /help to see available commands.';
      } else {
  ...
  return (
    <div className="w-[400px] h-[600px] bg-gray-900 rounded-2xl shadow-xl
                  border border-white/10 overflow-hidden flex flex-col">
      {/* Header */}
      <div className="p-4 bg-gradient-to-r from-[#FF6F3C] via-[#FFA833] to-[#FFC76D]
                    relative">
